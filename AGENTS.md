...
- `www.balcheck.in` has no DNS record at all (only apex AAAA, proxied) — anything that resolves www against the apex IP returns a 530 masquerading as a 404/5xx crawl error; fix: treat www as a DNS/route change needing approval, and note `src/worker.ts` already folds `www.*` onto the apex once a record exists.
